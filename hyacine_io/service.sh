#!/system/bin/sh
# Hyacine IO v1.4.0 - Runtime I/O + storage + USB hotplug
...
wv() {
    [ -w "$1" ] || { log "  $3: SKIP (not writable)"; return 1; }
    echo "$2" > "$1" 2>/dev/null
    [ "$(cat "$1" 2>/dev/null)" = "$2" ] && return 0
    log "  $3: FAIL (got: $(cat "$1" 2>/dev/null))"; return 1
}

while [ "$(getprop sys.boot_completed)" != "1" ]; do sleep 5; done

log "=== Hyacine IO v1.4.0 Active ==="
...
for vol in /storage/*; do
    ...
    timeout 10 am broadcast -a android.intent.action.MEDIA_SCANNER_SCAN_FILE -d "file:///storage/$VN" -p com.android.providers.media.module 2>/dev/null
    timeout 10 am broadcast -a android.intent.action.MEDIA_SCANNER_SCAN_FILE -d "file:///storage/$VN" -p com.android.providers.media 2>/dev/null
    log "SD card scanned: $VN"
    break
done

# USB hotplug (check every 300s, not 60s — save battery)
# Init LAST_USB from current count to detect first plug-in
LAST_USB=0
for dev in /sys/block/sd*; do [ -d "$dev" ] && LAST_USB=$((LAST_USB + 1)); done
while true; do
    sleep 300
    USB=0
    for dev in /sys/block/sd*; do [ -d "$dev" ] && USB=$((USB + 1)); done
    if [ "$USB" -gt "$LAST_USB" ]; then
        log "USB detected ($LAST_USB -> $USB)"
        timeout 10 am broadcast -a android.intent.action.MEDIA_MOUNTED -d file:///storage/emulated/0 -p com.android.providers.media.module --user 0 >/dev/null 2>&1
        timeout 10 am broadcast -a android.intent.action.MEDIA_MOUNTED -d file:///storage/emulated/0 -p com.android.providers.media --user 0 >/dev/null 2>&1
    fi
    LAST_USB=$USB
done
